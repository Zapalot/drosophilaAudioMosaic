...
def visualizeComponentsNMF(V, W, H, compV=None, parameter=None):
    ...
    W: array-like
        Numpy array with R indiviual K X T learned template matrices TODO

    H: array-like
        R X M matrix of learned activations

    compV: array-like
        numpy array with R individual component magnitude spectrograms TODO
    ...
    if R > 10:
        ...
        for r in range(R):
            ...
            xcoord = np.concatenate([timeAxis.reshape(1, -1), np.fliplr(timeAxis.reshape(1, -1))], axis=1)
    ...
    if R > 10:
        ...
        if normW.ndim > 2:
            normW = normW.squeeze(axis=2).T

        ax3.imshow(normW, aspect='auto', cmap='gray_r', origin='lower',
                   extent=[1, R, subSampFreqAxis[0], subSampFreqAxis[-1]])
        ...
        for r in range(R):
            ...
            if R <= 4:
                ...
                currTemplate = currTemplate.reshape(-1, 1)
