...
def inverseSTFT(X, parameter):
    ...
    analysisWinFunc = parameter['winFunc']
    synthesisWinFunc = parameter['winFunc']
    ...
    y = np.zeros(numSamples, dtype=np.complex32) if analyticSig else np.zeros(numSamples, dtype=np.float32)
    ...
    for k in range(numFrames):
        ...
        if reconstMirror:
            ...
            if not analyticSig:
                padMirrorSpec = np.conjugate(np.flip(currSpec[1:numBins - 1], axis=0))
